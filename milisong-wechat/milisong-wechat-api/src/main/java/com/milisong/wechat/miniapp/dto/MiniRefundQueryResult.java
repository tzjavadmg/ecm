...
@NoArgsConstructor
@XStreamAlias("xml")
public class MiniRefundQueryResult extends BasicPayDto implements Serializable {

    private static final long serialVersionUID = 6799138293861690934L;
    /**
     * <pre>
     * 字段名：微信订单号.
     * 变量名：transaction_id
    ...
    public void composeRefundRecords() {
        if (this.refundCount != null && this.refundCount > 0) {
            ...
            for (int i = 0; i < this.refundCount; i++) {
                ...
                List<MiniPayRefundCouponInfo> coupons = Lists.newArrayList();
                for (int j = 0; j < refundRecord.getCouponRefundCount(); j++) {
                    coupons.add(
                            new MiniPayRefundCouponInfo(
    ...
    public static class RefundRecord {
        ...
        private List<MiniPayRefundCouponInfo> refundCoupons;
