...
@XStreamAlias("xml")
@Data
public class PayUnifiedOrderRequest extends BasicPayDto {
    ...
    public PayUnifiedOrderRequest() {
    }

    private PayUnifiedOrderRequest(Builder builder) {
    ...
    public Map<String,String> toMap(){
        ...
        if (!isNullOrEmpty(deviceInfo)){
            map.put("device_info",deviceInfo.trim());
        }
        ...
        if (!isNullOrEmpty(detail)){
            map.put("detail",detail.trim());
        }

        if (!isNullOrEmpty(attach)){
            map.put("attach",attach.trim());
        }

        if (!isNullOrEmpty(outTradeNo)){
            map.put("out_trade_no",outTradeNo.trim());
        }

        if (!isNullOrEmpty(feeType)){
            map.put("fee_type",feeType.trim());
        }

        if (!isIllegalNumber(totalFee)){
            map.put("total_fee", convertToWxAmt(totalFee));
        }

        if (!isNullOrEmpty(spbillCreateIp)){
            map.put("spbill_create_ip",spbillCreateIp.trim());
        }

        if (!isNullOrEmpty(timeStart)){
            map.put("time_start",timeStart.trim());
        }

        if (!isNullOrEmpty(timeExpire)){
            map.put("time_expire",timeExpire.trim());
        }

        if (!isNullOrEmpty(goodsTag)){
            map.put("goods_tag",goodsTag.trim());
        }

        if (!isNullOrEmpty(notifyURL)){
            map.put("notify_url",notifyURL.trim());
        }

        if (!isNullOrEmpty(tradeType)){
            map.put("trade_type",tradeType.trim());
        }

        if (!isNullOrEmpty(productId)){
            map.put("product_id",productId.trim());
        }

        if (!isNullOrEmpty(limitPay)){
            map.put("limit_pay",limitPay.trim());
        }
        ...
        if (!isNullOrEmpty(subOpenid)){
            map.put("sub_openid",subOpenid.trim());
        }

        if (!isNullOrEmpty(sceneInfo)){
            map.put("scene_info",sceneInfo.trim());
        }
        return map;
    }
    ...
    private Boolean isIllegalNumber(Integer num){
        if (num == null || num <= 0){
            return true;
        }
        return false;
    ...
    private Boolean isIllegalNumber(BigDecimal num){
    ...
    private static String convertToWxAmt(BigDecimal transAmt) {
    ...
    public static final class Builder {
        ...
        public PayUnifiedOrderRequest build() {
            return new PayUnifiedOrderRequest(this);
        }
    }
}
