			...
			if (resultMap != null && resultMap.size() > 0 ) {
				for (Map.Entry<String, Map<Long,List<String>>> map : resultMap.entrySet()) {
					...
					for (Map.Entry<Long, List<String>> cataMap : value.entrySet()) {
						...
						String salableShopGoodsKey = RedisKeyUtils.getSalableShopGoodsKey(shopCode, date);
			            RedisCache.set(salableShopGoodsKey, JSON.toJSONString(goodsCataList));
			            RedisCache.expireAt(salableShopGoodsKey, DateUtils.addDays(date, 1));
					}
				}
			}
