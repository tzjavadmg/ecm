...
@Slf4j
@Service
public class LCRestPromotionService {
...
//    }
     ...
     * @return
     */
    public ResponseResult<LCUserRedPacketResponse> recevieMultiShareRedPacket(Long orderid) {
        UserSession userSession = UserContext.getCurrentUser();
        UserRedPacketRequest redPacketRequest = UserRedPacketRequest.builder().userId(userSession.getId()).openid(userSession.getOpenId()).
                isNewUser(userSession.getIsNew()).nickName(userSession.getNickName())
                .headPortraitUrl(userSession.getHeadPortraitUrl()).launchType(ORDER_SHARE).launchId(orderid).build();
        ResponseResult<LCUserRedPacketResponse> response = activityUserRedPacketService.recevieLCMultiShareRedPacket(redPacketRequest);
        LCUserRedPacketResponse userRedPacketResponse = response.getData();
        Byte isNew = userSession.getIsNew();
        Byte receivedNewRedPacket = userSession.getReceivedNewRedPacket();
        if ((isNew == null || isNew.intValue() == 0)) {//新用户领取新人红包
            Boolean receivedNewCouponFlag = false;
            Boolean receivedNewRedPacketFlag = false;
            if (userRedPacketResponse != null){
                receivedNewCouponFlag = userRedPacketResponse.getReceivedCoupon();
                if ((receivedNewRedPacket == null || receivedNewRedPacket.intValue() == 0) && userRedPacketResponse.getNewRedPacket() != null){
                    receivedNewRedPacketFlag = true;
                }

            }

            updateReceivedNewRedPacketStatus(userSession.getId(), userSession.getOpenId(),receivedNewRedPacketFlag,receivedNewCouponFlag);
        }
        return response;
    }

    // 修改用户领取红包状态
    private void updateReceivedNewRedPacketStatus(Long userId, String openid,Boolean receivedNewRedPacketFlag,Boolean receivedNewCouponFlag) {
        UserDto user = new UserDto();
        user.setId(userId);
        user.setOpenId(openid);
        if (Boolean.TRUE.equals(receivedNewRedPacketFlag)){
            user.setReceivedNewRedPacket((byte) 1);
            UserContext.freshUser(openid, UserSourceEnum.MINI_APP.getCode(),BusinessLineEnum.BREAKFAST.getCode(), "receivedNewRedPacket", "1");
        }

        if (Boolean.TRUE.equals(receivedNewCouponFlag)){
            user.setReceivedNewCoupon((byte) 1);
            UserContext.freshUser(openid, UserSourceEnum.MINI_APP.getCode(),BusinessLineEnum.BREAKFAST.getCode(), "receivedNewCoupon", "1");
        }

        user.setUpdateTime(new Date());
        user.setBusinessLine(BusinessLineEnum.BREAKFAST.getCode());
     ...
     * @return
     */
    public ResponseResult<Pagination<UserRedPacketDto>> myRedpacket(ActivityQueryParam queryParam) {
