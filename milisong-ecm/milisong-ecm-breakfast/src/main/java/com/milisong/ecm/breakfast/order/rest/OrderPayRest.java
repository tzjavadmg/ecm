...
import com.milisong.ecm.breakfast.order.service.OrderHelper;
import com.milisong.ecm.breakfast.order.service.OrderValidator;
import com.milisong.ecm.breakfast.order.service.VirtualOrderValidator;
...
//        }
        ...
        //--------业务线标识---------------
        virtualOrderParam.setOrderType(OrderType.BREAKFAST.getValue());
    ...
    ResponseResult<?> wechatMiniPay(@RequestBody OrderPaymentParam orderPaymentParam, HttpServletRequest request) {
        ...
        //--------业务线标识---------------
        orderPaymentParam.setOrderType(OrderType.BREAKFAST.getValue());
...
//        orderValidator.validate(orderPaymentParam);
    ...
    @PostMapping("/v1/order/payment-result")
    ResponseResult<?> paymentResult(@RequestBody PaymentResultParam paymentResultParam) {
        log.info("订单支付结果入参：{}", JSON.toJSONString(paymentResultParam));
        return orderPayService.paymentResult(paymentResultParam);
    }
