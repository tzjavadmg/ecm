...
import com.milisong.ecm.lunch.order.service.OrderHelperLC;
import com.milisong.ecm.lunch.order.service.OrderValidatorLC;
import com.milisong.ecm.lunch.order.service.VirtualOrderValidatorLC;
...
public class OrderPayRestLC {
    ...
    @Resource
    private OrderHelperLC orderHelperLC;
    @Resource
    private VirtualOrderValidatorLC virtualOrderValidatorLC;
    @Resource
    private OrderValidatorLC orderValidatorLC;
    /**
     * 去结算，生成虚拟订单，锁定库存
     *
    ...
    ResponseResult<?> virtualOrder(@RequestBody VirtualOrderParam virtualOrderParam) {
        ...
        ResponseResult<?> validateResult = virtualOrderValidatorLC.validate(virtualOrderParam);
        if (!validateResult.isSuccess()) {
            return validateResult;
        }

        log.info("创建虚拟订单参数：{}", JSON.toJSONString(virtualOrderParam));
        orderHelperLC.fillVirtualOrderParam(virtualOrderParam);
        //--------业务线标识---------------
    ...
    ResponseResult<?> wechatMiniPay(@RequestBody OrderPaymentParam orderPaymentParam, HttpServletRequest request) {
        ...
        orderPaymentParam.setOrderSource(OrderUserSource.WECHAT_MINI_BF.getValue());
        orderHelperLC.fillOrderPaymentParam(orderPaymentParam);
        log.info("支付参数：{}", JSON.toJSONString(orderPaymentParam));
        orderValidatorLC.validate(orderPaymentParam);
