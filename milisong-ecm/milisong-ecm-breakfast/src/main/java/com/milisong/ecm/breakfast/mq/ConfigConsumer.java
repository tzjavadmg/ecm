...
import com.milisong.ecm.common.dto.BannerConfigDto;
import com.milisong.ecm.common.dto.InterceptConfigDto;
import com.milisong.ecm.common.dto.SingleConfigDto;
...
public class ConfigConsumer {
	...
	private void global(String data) {
		...
		for(Map.Entry<String, Object> result :map.entrySet()) {
			RedisCache.hPut(globalConfigKey, result.getKey().trim(), result.getValue());
		}
	}
	...
	public void single(String data) {
		...
		if (CollectionUtils.isNotEmpty(singleList)) {
			for (SingleConfigDto single : singleList) {
				...
				RedisCache.hPut(shopConfigKey, single.getConfigKey().trim(), configValue);

				// 全局配置，保证在shopinfo接口没有拿到shopcode时候返回默认信息
				if (single.getShopCode().equals(SHOP_DEFAULT_CODE)) {
					String configKey = RedisKeyUtils.getGlobalConfigKey();
					RedisCache.hPut(configKey, single.getConfigKey().trim(), single.getConfigValue());
				}
			}
		}
	}
