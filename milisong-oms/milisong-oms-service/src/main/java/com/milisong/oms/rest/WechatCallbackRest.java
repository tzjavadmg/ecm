...
@Slf4j
@RestController
public class WechatCallbackRest {
     ...
     */
    @Deprecated
    @RequestMapping(value = "/v2/order/unifiedorder-callback-pre")
    public void unifiedOrderCallbackPre(HttpServletRequest request, HttpServletResponse response) {
        unifiedOrderCallback(request, response);
    }

    @RequestMapping(value = "/v2/order/unifiedorder-callback")
    public void unifiedOrderCallback(HttpServletRequest request, HttpServletResponse response) {
        try {
            String notityXml = getXml(request);
            log.info("主动支付回调 notityXml -> {}", notityXml);
    ...
    private void callback(String notifyXml) {
        ...
        if (StringUtils.isNotEmpty(orderIdString)) {
            ...
            orderPaymentHelper.paymentResult(paymentResultParam);
        } else {
    ...
    void refundCallback(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            SortedMap<String, String> map = XMLUtil.doXMLParse(notityXml);
            // 回调操作
            refundOrderService.refundCallback(map);
            response.getWriter().write(CALLBACK_SUCCESS);
        } catch (Exception e) {
            log.error("退款回调", e);
        }
